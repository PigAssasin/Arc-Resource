// Chọn filter "Video" → F12 → Console → Paste → Enter (KHÔNG cần scroll trước)
(async function () {
  const TYPE = 'Video';
  const URL_PATTERN = '/home/videos/';

  const dateRe = /(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d+(?:st|nd|rd|th)?,?\s*\d{4}/i;
  const collected = new Map(); // href → data row
  ...
  function collectNow() {
    document.querySelectorAll('a[href*="' + URL_PATTERN + '"]').forEach(link => {
      ...
      const author = (dateIdx > 0 && lines[dateIdx - 1].length < 60) ? lines[dateIdx - 1] : '';
      const tags = [...new Set(text.match(/#[\w-]+/g) || [])].join(', ');
      const cm = text.match(/(\d+)\s*(?:comment|repl)/i);

      collected.set(link.href, [0, TYPE, date, title, author, tags, cm ? cm[1] : '', link.href]);
    });
  }

  // Scroll từng bước, thu thập tại mỗi vị trí
  ...
  while (noNewCount < 6) {
    ...
    console.log('Scroll pos:', pos, '| Items:', nowSize);
    if (pos > document.body.scrollHeight + window.innerHeight * 2) break;
  }

  // Thu thập lần cuối ở đáy
  ...
  if (!results.length) { alert('Khong tim duoc item nao!'); return; }

  const tsv = ['STT\tLoai\tNgay\tTieu de\tTac gia\tTags\tBinh luan\tURL',
    ...results.map(r => r.join('\t'))].join('\n');

  // Hiển thị overlay với nút Copy (dùng user gesture để clipboard hoạt động)
  ...
  copyBtn.style.cssText = 'flex:1;padding:12px;background:#0070f3;color:#fff;border:none;border-radius:6px;font-size:15px;cursor:pointer;font-weight:bold';
  copyBtn.onclick = () => {
    ta2.select();
