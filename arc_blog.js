// Chọn filter "Blog" → F12 → Console → Paste → Enter (KHÔNG cần scroll trước)
(async function () {
  const TYPE = 'Blog';
  const URL_PATTERN = '/home/blogs/';
  ...
  if (!results.length) { alert('Khong tim duoc Blog nao!'); return; }

  const tsv = ['STT\tLoai\tNgay\tTieu de\tURL',
  ...
  copyBtn.style.cssText = 'flex:1;padding:12px;background:#22c55e;color:#fff;border:none;border-radius:6px;font-size:15px;cursor:pointer;font-weight:bold';
  copyBtn.onclick = () => {
    ta2.select();
